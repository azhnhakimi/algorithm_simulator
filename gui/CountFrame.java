...
public class CountFrame extends JPanel {

    List<Integer> stackHistory;
    
    public CountFrame(List<Integer> stackHistory){
        this.setOpaque(true);
        ...
        for(int i = 0; i <= stackHistory.size(); i++){
            ...
            if(i == 0 || i == stackHistory.size()){
                Seperator countTile = new Seperator(0);
                this.add(countTile);
                if(i != 0){
                    break;
                }
            ...
            CountTile countTile = new CountTile(width);
            this.add(countTile);
        }  
    }
